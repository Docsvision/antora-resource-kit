...
В данном разделе приведена инструкция по установке {es} на Windows и Linux. Перед установкой ознакомьтесь с требованиями системы {es} в разделе по ссылке "xref:new-admin-console@install-linux:ROOT:requirements-hardware.adoc#elastic[]" для Linux и
"xref:new-admin-console@installguide:ROOT:requirements-hardware.adoc#elastic[]" для Windows.
...
+
* Для ОС Windows загрузите инсталлятор https://www.elastic.co/downloads/past-releases/elasticsearch-{es-v}[с сайта {es}] или https://cloud.mail.ru/public/EA3t/5VLu9GyGj[по данной ссылке] и установите на компьютер. При установке не изменяйте настройки по умолчанию.
+
...
ОС Linux::
----
sudo /usr/share/elasticsearch/bin/elasticsearch-plugin install --batch "file://$(realpath ingest-attachment-7.14.0.zip)" <.>
----
...
. Используйте образ `packages.docsvision.com/elasticsearch:7.14.0` или соберите его самостоятельно:
+
[source,dockerfile]
----
